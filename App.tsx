...
import { fetchWithRetry } from './utils/api'; // Importação do Retry
...
const App: React.FC = () => {
  ...
  const [internationReportData, setInternationReportData] = useState<InternationSheetRowData[]>([]); // Novo State
  ...
  const [discriminators, setDiscriminators] = useState<CtasDiscriminators>({
    ...
    cardio: { chestPainRisk: false, severePainWithVitals: false },
  ...
  useEffect(() => {
    ...
    setPatient(prev => ({ 
      ...prev, 
      serviceTimestamp: now.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
  ...
  // Sincroniza dados da Triagem (Aba Pacientes)
  ...
  useEffect(() => {
    if (isAuthenticated) {
        ...
        if (activeTab === 'internationReports' || activeTab === 'dashboard') {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && activeTab === 'dashboard') {
      interval = setInterval(() => {
        handleSyncFromSheet();
        handleSyncInternation();
      }, 30 * 1000); // ATUALIZADO PARA 30 SEGUNDOS
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const safePayload = {
      ...
      patient: {
          ...patient,
          evaluationDate: finalEvaluationDate,
          evaluationTime: finalEvaluationTime,
      ...
      triage: {
          level: String(triageResult.level),
          title: triageResult.title,
          maxWaitTime: triageResult.maxWaitTime,
          justification: triageResult.justification,
          discriminators: triageResult.discriminators
      },
      discriminators,
      // ENVIO DE RASTREABILIDADE (TRIAGEM) - PRIORIDADE PARA O NOME
    ...
    try {
      ...
      setPatient({
        name: '', medicalRecord: '', dob: '', serviceTimestamp: currentTime, 
        evaluationDate: currentDate, evaluationTime: currentTime,
        isReevaluation: false, age: 0, ageUnit: 'years', gender: 'M', complaint: '',
        reevaluationDate: currentDate, reevaluationTime: currentTime
      });
      setVitals({ pas: '', pad: '', fc: '', fr: '', temp: '', spo2: '', gcs: 15, painLevel: '' });
      setDiscriminators({
        abcUnstable: false, highRiskSituation: false, resources: 'none',
        neuro: { gcsLow: false, acuteConfusion: false, headTrauma: false, severeHeadache: false },
        sepsis: { suspectedInfection: false, immunosuppressed: false, perfursionIssues: false },
        cardio: { chestPainRisk: false, severePainWithVitals: false },
        respiratory: { dyspneaRisk: false, respiratoryDistress: false },
  ...
  const handleExportExcel = useCallback((worsened: any[], month: string) => {
    if (worsened.length === 0) {
      setNotification({ msg: 'Não há dados de piora clínica para exportar.', type: 'error' });
      setTimeout(() => setNotification(null), 3000);
      return;
    }
    // @ts-ignore
    if (typeof XLSX === 'undefined') {
      setNotification({ msg: 'Biblioteca Excel não carregada. Tente recarregar a página.', type: 'error' });
      return;
    }
    const dataToExport = worsened.map(item => ({
      ...
      'Data da Reavaliação': new Date(item.date).toLocaleDateString('pt-BR')
    }));
    // @ts-ignore
    ...
    <>
      {isGeneratingPdf && (
        <div className="fixed inset-0 z-[100] bg-slate-900 flex flex-col items-center justify-center text-white">
          ...
          <h2 className="text-2xl font-bold">Gerando Relatório...</h2>
        </div>
      )}
      ...
      {isGeneratingPdf && activeTab === 'triage' && (
        <div className="fixed top-0 left-0 w-full h-full bg-white z-[50] flex justify-center items-start overflow-auto">
          <div id="report-content" style={{ width: '794px', height: '1120px', padding: '30px', backgroundColor: 'white', boxSizing: 'border-box' }}>
      ...
      <div className={`min-h-screen bg-slate-100 font-sans pb-20 ${isGeneratingPdf ? 'hidden' : 'block'}`}>
        {notification && (
           <div className={`fixed top-4 right-4 p-4 rounded-md shadow-lg text-white font-medium z-50 animate-bounce ${notification.type === 'success' ? 'bg-teal-600' : 'bg-rose-600'}`}>
             {notification.msg}
           </div>
        )}
        ...
        <header className="bg-white border-b border-slate-300 shadow-sm sticky top-0 z-20">
           <div className="max-w-6xl mx-auto px-4 py-3">
              <div className="flex flex-col md:flex-row md:justify-between items-center mb-4 gap-4">
                  <div className="flex items-center justify-between w-full md:w-auto">
                    <div className="flex items-center gap-3">
                        <Activity className="text-rose-600 shrink-0" size={28} />
                        <div className="flex flex-col">
                            <h1 className="text-lg font-bold text-slate-800 uppercase tracking-tight leading-none">
                                Triagem Híbrida <span className="text-rose-600">ESI + CTAS</span>
                            </h1>
                            <span className="text-[10px] font-bold text-emerald-600 tracking-widest uppercase mt-0.5">
                                + NEWS Score Monitoramento
                            </span>
                        </div>
                    </div>
                    <button onClick={handleLogout} className="md:hidden text-slate-400 hover:text-rose-600"><LogOut size={20} /></button>
                  </div>

                  <div className="flex items-center gap-4">
                    {activeTab === 'triage' && (
                      <div className={`px-4 py-2 rounded font-bold text-white shadow-sm transition-colors uppercase tracking-wider text-sm ${triageResult.color}`}>
                          {triageResult.title}
                      </div>
                    )}
                    <div className="hidden md:flex items-center gap-3 pl-4 border-l border-slate-200">
                       ...
                       <button onClick={handleLogout} className="bg-slate-100 p-2 rounded-full hover:bg-rose-50 hover:text-rose-600 transition-colors" title="Sair"><LogOut size={18} /></button>
                    </div>
                  </div>
              </div>
              <div className="flex space-x-4 border-b border-slate-200 overflow-x-auto">
                 ...
                 <button onClick={() => setActiveTab('internation')} className={`pb-2 px-4 font-medium text-sm transition-colors border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'internation' ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    <BedDouble size={16}/> Pacientes Internados
                 </button>
                 ...
                 {/* NOVA ABA INDICADORES INTERNAÇÃO */}
                 <button onClick={() => setActiveTab('internationReports')} className={`pb-2 px-4 font-medium text-sm transition-colors border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'internationReports' ? 'border-sky-600 text-sky-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    <BarChart2 size={16}/> Indicadores Internação
                 </button>
        ...
        <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
          <React.Suspense fallback={<div className="flex justify-center p-10"><div className="animate-spin rounded-full h-10 w-10 border-4 border-teal-500 border-t-transparent"></div></div>}>
            {activeTab === 'triage' && (
              <TriageSection 
                patient={patient}
            ...
            {/* RENDERIZAÇÃO RELATÓRIO INTERNAÇÃO (NOVO) */}
            {activeTab === 'internationReports' && (
                <InternationReportsSection 
                    reportData={internationReportData}
                    handleSyncFromSheet={handleSyncInternation}
            ...
            {activeTab === 'dashboard' && (
              <DashboardSection 
                ...
                internationData={internationReportData}
        ...
        <button 
          ...
          className="fixed bottom-20 right-4 p-3 bg-slate-700 hover:bg-slate-800 rounded-full text-white shadow-lg z-10"
        >
          <HelpCircle size={24} />
        </button>
        ...
        {isTechSheetOpen && (
          <React.Suspense fallback={null}>
            <TechnicalSheetModal isOpen={isTechSheetOpen} onClose={() => setIsTechSheetOpen(false)} />
          </React.Suspense>
        )}
